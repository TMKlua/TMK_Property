...
window.addEventListener("message", function(event) {
    if (event.data.action == "display") {
        ...
        $.post("http://cProperty/GiveItem", JSON.stringify({
...
function closeInventory() {
    $.post("http://cProperty/NUIFocusOff", JSON.stringify({}));
}
...
$(document).ready(function() {
    ...
    $(document).mousedown(function(event) {
        ...
        if (itemInventory == "second") {
            $.post("http://cProperty/TakeFromChest", JSON.stringify({
                item: itemData,
    ...
    $(document).mousedown(function(event) {
        ...
        if (itemInventory == "main") {
            $.post("http://cProperty/PutIntoChest", JSON.stringify({
                item: itemData,
        ...
        drop: function(event, ui) {
            ...
            if (type === "Chest") {
                ...
                if (itemData.type === "item_weapon") {
                    ...
                    $.post("http://cProperty/TakeFromFast", JSON.stringify({
                        item: itemData
                    }));
                }
            }
        ...
        drop: function(event, ui) {
            ...
            if (type === "Chest" && itemInventory === "main") {
                ...
                if (itemData.type === "item_weapon") {
                    disableInventory(500);
                    Hotbar[itemData.slot - 1] = false;
                    $.post("http://cProperty/PutIntoFast", JSON.stringify({
                        item: itemData
                    }));
                }
            }
